...
verify_asset_path() {
    ...
    case "$expected_type" in
        directory)
            [[ -d "$path" ]] && actual="directory"
            ;;
        file)
            [[ -f "$path" ]] && actual="file"
            ;;
        *)
            fail "Unknown asset verification type: $expected_type"
            ;;
    esac
    ...
    if [[ "$actual" == "$expected_type" ]]; then
        ...
        warn "Appearance asset is missing: $subject ($path)"
        VERIFY_FAILURES=$((VERIFY_FAILURES + 1))
    fi
...
verify_configuration() {
    ...
    icon_theme_dir="$(find_icon_theme_directory)"
    verify_asset_path "GTK/Shell theme directory" "$theme_dir" directory
    verify_asset_path \
        "GTK 3 theme stylesheet" \
    ...
    verify_asset_path \
        ...
        file
    verify_asset_path "Papirus icon theme" "$icon_theme_dir/index.theme" file

    if [[ -f "$TARGET_HOME/.config/gtk-4.0/gtk.css" ]]; then
        verify_asset_path \
            "User GTK 4 stylesheet" \
            "$TARGET_HOME/.config/gtk-4.0/gtk.css" \
            file
    elif [[ -f "$theme_dir/gtk-4.0/gtk.css" ]]; then
        verify_asset_path \
            "Theme GTK 4 stylesheet" \
...
main() {
    ...
    # Reassert the curated state after the importer and regenerate the detailed
    # extension inventory. No extension code is downloaded in state-only mode.
    bash "$SCRIPT_DIR/04-setup-extensions.sh" --state-only \
        2>&1 | tee -a "$LOG_FILE"
