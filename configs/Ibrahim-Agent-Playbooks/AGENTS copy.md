...
Prefer native PowerShell and Windows OpenSSH where available.
...
Prefer robust Bash scripts using:
...
When a command is intended to be copied directly, prefer a **single-line command** unless multiline syntax is genuinely necessary.

For Fin-Glassbox in particular, do not use backslash-separated multiline CLI commands.
...
Do not silently generate dummy data, fake embeddings, synthetic labels, or placeholder model outputs in production paths.
...
A resume mechanism is not complete merely because a checkpoint file exists.
